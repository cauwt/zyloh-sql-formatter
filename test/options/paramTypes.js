...
exports.default = supportsParamTypes;
const dedent_js_1 = __importDefault(require("dedent-js"));
function supportsParamTypes(format) {
    ...
    describe('when paramTypes.custom=[...]', () => {
      ...
      `);
        ...
        it('does not enter infinite loop when empty regex given', () => {
            expect(() => format('SELECT foo FROM bar', {
                paramTypes: { custom: [{ regex: '' }] },
            })).toThrow('Empty regex given in custom paramTypes. That would result in matching infinite amount of parameters.');
        });
    });
}
